...
if [ $# -lt 1 ] ; then
  echo "Usage: anu_skt_hnd.sh <file> tmp_dir_path hi [DEV|ROMAN|VH] [NO|YES] [UoHyd|GH] [NO|Partial|Full] [ECHO|NOECHO] [D]."
fi
...
$ANU_MT_PATH/morph/morph.sh $temp_files_path/$fbn.out $temp_files_path/$fbn.mo_all $temp_files_path/$fbn.mo_prune $temp_files_path/$fbn.mo_kqw
       if [ $DEBUG = "OFF" ]; then 
  ...
  if [ $MORPH = "GH" ] ; then
     Heritage_Input="YES"
     HERITAGE_PATHML/reader_plugin < $TMP_DIR_PATH/$1 > $temp_files_path/$1.gh
