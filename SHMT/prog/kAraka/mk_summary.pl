...
my $SCRIPT=$ARGV[0];
my $filename=$ARGV[1];
my $kAraka_name=$ARGV[2]."/kAraka_name_".lc($SCRIPT).".gdbm";
my $dirname=$ARGV[3];
my $relations=$ARGV[4];
my $sentnum=$ARGV[5];
my $OUTFILE=$ARGV[6];
...
require "$myPATH/SHMT/prog/interface/modify_mo_fn_".lc($SCRIPT).".pl";
 ...
 print "<td><ol>\n";
    while($COL{$i} =~ /^([^;]+);/) {
      ...
      #    }
          ...
          print "<a href=\"CGIURL/SHMT/prog/interface/call_parser_summary.cgi?filename=$dirname&amp;outscript=$SCRIPT&amp;rel=$rels&amp;sentnum=$sentnum&amp;save=no&amp;translate=no\" title=\"$morph{$mindx}\">";
          print " &#x2713; $rel </a> <\/li>\n";
         }
       }
    }
 ...
 for($i=0; $i <= $tot_words; $i++){
         ...
         if($morph{$mindx}) { # assuming that there are < 10 morph solns
           ...
           if( $exists{$mindx} ){
               ...
               print "<li class=\"$class\">";
               print $j+1,"<a href=\"CGIURL/SHMT/prog/interface/call_parser_summary.cgi?filename=$dirname&amp;outscript=$SCRIPT&amp;rel=$rels&amp;sentnum=$sentnum&amp;save=no&amp;translate=no\" title=\"$morph{$mindx}\">&#x2713; $morph{$mindx} </a> <\/li>\n";
           }
         }
           $j++;
           $mindx = "$i,$j";
...
1;
 ...
 my($soln,$total_solns,$dirname,$SCRIPT,$old_relations,$sentnum) = @_;
      ...
      if(($total_solns == 1) || ($soln == 1)){
         ...
         print "<a href=\"CGIURL/SHMT/prog/interface/show_selected_parse.cgi?filename=$dirname/&amp;sentnum=$sentnum&amp;start=0&amp;outscript=$SCRIPT\"> &#x2713; <\/a>Unique parse tree \n";
