...
//		cout<<"done splitting\n"; //Anil
		...
		no_of_splits.clear();
		rule_applied.clear();
		string command="LTPROCBINDIR/lt-proc -c "+(string)argv[5]+" < temp_result > temp_result_mo; grep '*' temp_result_mo > todelete; sed 's/[^\\/]\\+sa-u-pa[^\\/]\\+//g' temp_result_mo | grep '\\/$'>> todelete";
