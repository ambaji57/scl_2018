...
require "SCLINSTALLDIR/SHMT/prog/morph/web_interface/scripts.pl";
...
$ans = `SCLINSTALLDIR/SHMT/prog/morph/webrun_morph.sh $word_wx`;
chomp($ans);
...
if($ans ne "") {
 ...
 # Why only level 4 entry iin the beginning is deleted, why not all?
 ...
 # We need to separate the upasarga from the rts for generation purpose.
   ...
   if($rt ne $word) { #  To avoid the infinite regress
     if($ans =~ /कृत्_प्रत्ययः/ ){
       ...
       $color = "";
    } elsif($ans =~ /कृदन्त|तद्धित/) {
             $ans =~ s/{वर्गः ना}//;
             $link = "<a href=\"javascript:analyse_noun_forms('Unicode','$rt')\">$rt</a>";
       } elsif ($ans =~ /{वर्गः ना}/ ) {
                $ans =~ s/{वर्गः ना}//;
               if ($ans =~ /(पुं|नपुं|स्त्री)/){ $lifga = $1;}
                $link = "<a href=\"javascript:generate_noun_forms('Unicode','$rt','$lifga','nA','1')\">$rt</a>";
                $color = "skyblue";
      } elsif ($ans =~ /संख्या/)  {
         if ($ans =~ /(पुं|नपुं|स्त्री)/){ $lifga = $1;} else  {$lifga = "अ";}
         $link = "<a href=\"javascript:generate_noun_forms('Unicode','$rt','$lifga','saMKyA','1')\">$rt</a>";
         $color = "skyblue";
      } elsif ($ans =~ /संख्येय/) {
         if ($ans =~ /(पुं|नपुं|स्त्री)/){ $lifga = $1;} else  {$lifga = "अ";}
         $link = "<a href=\"javascript:generate_noun_forms('Unicode','$rt','$lifga','saMKyeyam','1')\">$rt</a>";
         $color = "skyblue";
      } elsif ($ans =~ /सर्वनाम/){
         if ($ans =~ /(पुं|नपुं|स्त्री)/){ $lifga = $1;} else  {$lifga = "अ";}
         $link = "<a href=\"javascript:generate_noun_forms('Unicode','$rt','$lifga','sarva','1')\">$rt</a>";
         $color = "skyblue";
