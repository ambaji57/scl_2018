...
cut -f4 | perl -pe 's/\-/\-\n-/g' | $myPATH/SHMT/prog/morph/bin/get_std_spelling.out |\
$myPATH/SHMT/prog/skt_lwg/group.pl |\
$myPATH/SHMT/prog/morph/bin/split-samAsa-wrds.pl $TMP_FILES_PATH/tmpall $TMP_FILES_PATH/tmpany $TMP_FILES_PATH/tmpsamb $TMP_FILES_PATH/tmpupaxa

#/usr/bin/time -f "%Uuser %Ssystem %Eelapsed %PCPU (%Xtext+%Ddata %Mmax)k\n%Iinputs+%Ooutputs (%Fmajor+%Rminor)pagefaults %Wswaps %C\n" 
LTPROCBINDIR/lt-proc -c $BIN_PATH/samAsa_upaxa.bin < $TMP_FILES_PATH/tmpupaxa | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/\$$//;' > $TMP_FILES_PATH/tmp_n1
LTPROCBINDIR/lt-proc -c $BIN_PATH/samboXana.bin < $TMP_FILES_PATH/tmpsamb | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/\$$//' > $TMP_FILES_PATH/tmp_n2
LTPROCBINDIR/lt-proc -c $BIN_PATH/all_morf.bin < $TMP_FILES_PATH/tmpany | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/\$$//;s/[a-zA-Z]+<vargaH:sa-pU-pa><lifgam:[^>]+><level:0>//g;s/>([a-zA-Z]+)</><kqw_pratipadika:\1></g;' > $TMP_FILES_PATH/tmp_n3
...
#if [ $w1 -gt 0 ]; then
#/usr/bin/time -f "\t%Uuser %Ssystem %Eelapsed %PCPU (%Xtext+%Ddata %Mmax)k\n%Iinputs+%Ooutputs (%Fmajor+%Rminor)pagefaults %Wswaps %C\n" 
#LTPROCBINDIR/lt-proc -c $BIN_PATH/skt_samAsanoun_morf.bin < $TMP_FILES_PATH/tmp_samAsa-wrds | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/^-$//;' > $TMP_FILES_PATH/tmp_samAsa
#LTPROCBINDIR/lt-proc -c $BIN_PATH/all_but_samboXana_morf_Apte.bin < $TMP_FILES_PATH/tmp_upada_samAsa-wrds | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/^-$//;' > $TMP_FILES_PATH/tmp_samAsa
#LTPROCBINDIR/lt-proc -c $BIN_PATH/skt_morf.bin < $TMP_FILES_PATH/tmp_samAsa-wrds | perl -pe 's/\//=/;s/^.*=\*.*//;s/.*=//;s/^^//;s/^-$//;' > $TMP_FILES_PATH/tmp_samAsa2
...
$myPATH/SHMT/prog/morph/bin/handle_Namul_next.pl < $TMP_FILES_PATH/tmp_n | $myPATH/SHMT/prog/morph/bin/handle_Namul_prev.pl  | perl -pe 's/=\//=/;s/\$$//g' > $TMP_FILES_PATH/tmp_nn
...
#if [ $w1 -gt 0 ]; then
##/usr/bin/time -f "\t%Uuser %Ssystem %Eelapsed %PCPU (%Xtext+%Ddata %Mmax)k\n%Iinputs+%Ooutputs (%Fmajor+%Rminor)pagefaults %Wswaps %C\n" 
#LTPROCBINDIR/lt-proc -c $BIN_PATH/kqw_lyap_morf.bin < $TMP_FILES_PATH/tmp_k_words |\
